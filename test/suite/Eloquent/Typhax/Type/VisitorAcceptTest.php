 ...
 */
class VisitorAcceptTypes extends PHPUnit_Framework_TestCase
{
    public function setUp()
    {
        $this->_visitor = Phake::mock(__NAMESPACE__ . '\Visitor');
    }
    ...
    public function testAccept($className)
    {
        ...
        Phake::when($this->_visitor)
            ->{'visit' . $className}($type)
            ->thenReturn('<visitor result>');

        $result = $type->accept($this->_visitor);
